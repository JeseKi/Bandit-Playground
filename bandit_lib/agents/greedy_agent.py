...
class GreedyAlgorithm(BaseAlgorithm["GreedyAgent"]):
    def __init__(self, config: AlgorithmConfig) -> None:
        super().__init__(config, GreedyAgent)
...
class GreedyAgent(BaseAgent[GreedyRewardStates, GreedyAlgorithm]):
    def __init__(
        ...
        self.rewards_states = GreedyRewardStates.create(env.arm_num, metrics_config.sliding_window_size)
    ...
    def update(self, arm_index: int, reward: int) -> None:
        ...
        if self.algorithm.config.enable_decay_alpha:
            q = old_q + self.algorithm.config.constant_step_decay_alpha * (reward - old_q)
        else:
